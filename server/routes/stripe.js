...
// Create subscription
router.post('/create-subscription', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const user = req.user;
...
router.post('/update-payment-method', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
...
// Cancel subscription
router.post('/cancel-subscription', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const user = req.user;
...
// Resume subscription
router.post('/resume-subscription', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const user = req.user;
...
router.post('/confirm-subscription-payment', passport.authenticate('jwt', { session: false }), async (req, res) => {
  try {
    const user = req.user;
    ...
    switch (event.type) {
      case 'invoice.payment_succeeded':
        ...
        if (invoice.subscription) {
          ...
          console.log(`Subscription ${invoice.subscription} activated via payment success`);
        }
        break;
      ...
      case 'invoice.payment_failed':
        ...
        if (failedInvoice.subscription) {
          ...
          console.log(`Subscription ${failedInvoice.subscription} marked as past due`);
        }
        break;
      ...
      case 'customer.subscription.updated':
        ...
        if (updatedSubscription.cancel_at_period_end && status === 'active') {
          status = 'canceled';
        } else if (!updatedSubscription.cancel_at_period_end && status === 'active') {
          status = 'active';
        }
        ...
        console.log(`Subscription ${updatedSubscription.id} updated to status: ${status}`);
        break;
      ...
      case 'customer.subscription.created':
        ...
        console.log(`Subscription ${createdSubscription.id} created with status: ${createdSubscription.status}`);
        break;
